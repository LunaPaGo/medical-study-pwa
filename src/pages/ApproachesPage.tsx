...
export function ApproachesPage() {
  ...
  const hypercalcemiaFixtureTitle = 'Hipercalcemia';
  ...
  const hasHypercalcemiaFixture = approaches.some((approach) => approach.title.trim().toLocaleLowerCase('es') === hypercalcemiaFixtureTitle.toLocaleLowerCase('es'));
  ...
  const loadHypercalcemiaFixture = async () => {
    if (!user?.id || hasHypercalcemiaFixture) return;
    setFixtureError('');
    try {
      const { createHypercalcemiaClinicalApproach } = await import('../features/approaches/hypercalcemiaApproachFixture');
      const saved = await mutations.save.mutateAsync(createHypercalcemiaClinicalApproach(user.id));
      navigate(`/abordajes/${saved.id}`);
    } catch (loadError) { setFixtureError(loadError instanceof Error ? loadError.message : 'No se pudo cargar el ejemplo en este dispositivo.'); }
  };

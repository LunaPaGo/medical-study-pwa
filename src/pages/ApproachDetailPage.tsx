...
import { getMemoryApproach } from '../features/approaches/clinicalApproachMemoryStore';
...
export function ApproachDetailPage() {
  ...
  const approach = approachId ? getMemoryApproach(approachId) : undefined;
  const [mode, setMode] = useState<ClinicalApproachViewMode>('study');
  if (!approach) return <Navigate to="/abordajes" replace />;

  return <article className="approach-reader">
    <header className="approach-header"><div><span className={`status-pill ${approach.status}`}>{approach.status === 'complete' ? 'Completo' : 'Borrador'}</span><h1>{approach.title}</h1><p>{approach.description}</p><div className="chip-list">{approach.category && <span className="tag-chip">{approach.category}</span>}{approach.tags.map((tag) => <span className="tag-chip" key={tag}>{tag}</span>)}</div></div><Link className="ghost-button" to={`/abordajes/${approach.id}/editar`}><Edit3 size={17} />Editar</Link></header>

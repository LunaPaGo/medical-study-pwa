...
function ApproachEditorForm({ initial }: { initial: ClinicalApproach }) {
  ...
  return <section className="page-stack">
    ...
    <form className="approach-form" onSubmit={submit}>
      ...
      <section className="approach-editor-shell"><div><h2>Secciones estructuradas</h2><p>Se conserva su modelo completo aunque todavía no tengan editores especializados.</p></div><ol>{clinicalApproachSections.filter((section) => !['presentation', 'initial-assessment', 'life-threats', 'initial-treatment', 'reassessment', 'warnings-and-instructions', 'common-errors', 'clinical-pearls'].includes(section.id)).map((section) => <li key={section.id}><span>{section.title}</span><small>Estructura preparada</small></li>)}</ol></section>

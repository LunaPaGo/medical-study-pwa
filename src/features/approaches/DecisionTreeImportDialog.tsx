  ...
  return <div className="modal-backdrop decision-tree-import-backdrop" role="dialog" aria-modal="true" aria-labelledby="decision-tree-import-title">
    <section className="preview-modal decision-tree-import-dialog">
      ...
      {result?.success && summary && <div className="decision-tree-import-preview">
        ...
        <div className="decision-tree-import-full-preview"><DecisionTreeFullView tree={result.tree} mode="quick" /></div>
      ...
      <footer className="decision-tree-import-actions"><button className="ghost-button" type="button" onClick={onClose}>Cancelar</button><button className="secondary-button" type="button" onClick={validate}><FileJson size={17} />Validar algoritmo</button><button className="primary-button" type="button" disabled={!result?.success} onClick={confirmImport}>Importar y reemplazar árbol actual</button></footer>
    </section>
  </div>;
}
